...
// Added functions for a2, added by Callum Johnson:

// Takes pointers to two deques and adds the contents of the second one 
// onto the bottom of the first without destroying the original second deque
void deque_merge_copy(Deque *top_deque, Deque *bottom_deque){
  int nElementsToAdd = deque_size(bottom_deque), data;
  for (int i = 0; i < nElementsToAdd; i++){
    // take off the first element of the bottom deque and add it to
    // the bottom of the top deque
    data = deque_pop(bottom_deque);
    deque_insert(top_deque,data);
    // Insert the data back into the bottom deque so as to preserve it
    deque_insert(bottom_deque, data);
  }
}

// Takes a deque and returns a copy of it
Deque *copy_of_deque(Deque *original_deque){
  Deque *copyDeque = new_deque();
  int nAdditions = deque_size(original_deque);
  for (int i = 0; i < nAdditions; i ++){
    int data = deque_pop(original_deque);
    deque_insert(copyDeque, data);
    deque_insert(original_deque, data);
  }
  return copyDeque;
}

// Takes an array of deques and frees it's contents
void free_deque_array(Deque **array, int size){
  for (int i = 0; i < size; i ++){
    free_deque(array[i]);
  }
}

// Empties a deque's contents without freeing the deque itself
