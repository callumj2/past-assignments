...
void apply_insertion(HashTable *hashTable, int index, Deque *string){
  deque_replace(hashTable->table[index], string);
}
...
void rehash(HashTable *hashTable, int newSize){
  ...
  HashTable *newTable = new_hash_table(newSize, hashTable->stepSize);
  ...
  for (int i = 0; i < oldSize; i ++){
    if (deque_size(oldTable[i]) > 0){
      hash_insert(newTable, oldTable[i]);
    }
  }
...
void free_hash_table(HashTable *hashTable){
  ...
  free_deque_array(hashTable->table, hashTable->tableSize);
